...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public immutable BNB = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); //BNB
...
contract SpiderBSCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SpiderBSCDividendTracker", "SpiderBSC_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SpiderBSCDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SpiderBSCDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main SpiderBSC contract.");
    }
...
contract SpiderBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SpiderBSCDividendTracker public dividendTracker;
    address private marketingWallet = 0xDf8Ff55E1D1392E3743C684029d95053a60a6071;
    ...
    constructor() ERC20("SpiderBSC", "SpiderBSC") {
        dividendTracker = new SpiderBSCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SpiderBSC: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SpiderBSC: The dividend tracker already has that address");

        SpiderBSCDividendTracker newDividendTracker = SpiderBSCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SpiderBSC: The new dividend tracker must be owned by the SpiderBSC token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SpiderBSC: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "SpiderBSC: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SpiderBSC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SpiderBSC: Cannot update gasForProcessing to same value");
    ...
    function swapAndDistributeETH(uint256 tokens) private {
        ...
        uint256 balance = IERC20(BNB).balanceOf(address(this));
        ...
        bool success = IERC20(BNB).transfer(marketingWallet, forMarketing);
        ...
        success = IERC20(BNB).transfer(address(dividendTracker), forDividends);
